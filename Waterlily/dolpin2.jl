...
sim_mem = Lava.LavaArray  # switch to ROCArray to run the sim on AMDGPU
cache_file = joinpath(@__DIR__, "dolphin_sim_$(sim_mem === ROCArray ? "rocm" : "lava").jld2")

# Rendering backend. Lava.LavaBackend() (HW RT available) or AMDGPU.ROCBackend() (SW BVH only).
render_device = Lava.LavaBackend()
render_device = AMDGPU.ROCBackend()  # --- IGNORE ---
render_hw_accel = render_device isa Lava.LavaBackend  # HW RT only on Lava
render_tag = render_device isa Lava.LavaBackend ? "lava" : "rocm"
output_file = joinpath(@__DIR__, "dolphin_4panel_$(render_tag).png")
...
function panel_streamlines(dmesh, vfunc, bounds, cam, domain_size; spp=256, bcx=0f0, bcy=0f0, bcz=0f0)
    (bxr, byr, bzr) = bounds
    ...
    s = Scene(size=(960,540);
        ...
        backgroundcolor=RGBf(0.03,0.05,0.14))
    cam3d!(s)
    mesh!(s, dmesh; material=Hikari.CoatedDiffuse(
        reflectance=(0.30f0,0.35f0,0.45f0), roughness=0.02f0, eta=1.5f0))
    streamplot!(s, vfunc, xm..xM, ym..yM, zm..zM;
        gridsize=(8,16,8), stepsize=0.35f0, maxsteps=1000, density=0.8,
        use_tubes=true, tube_radius=0.13f0, tube_n_sides=8,
        tube_spline=true, tube_spline_resolution=4,
        color=dx -> log10(max(1f-4, norm(dx))) + 4f0,
        colormap=:inferno, arrow_size=0)
    update_cam!(s, cam..., Vec3f(0,0,1))
    return colorbuffer(s; device=render_device,
        integrator=Hikari.VolPath(; samples=spp, max_depth=8, hw_accel=render_hw_accel),
        tonemap=:aces, gamma=2.2f0, exposure=2.0f0)
end

function panel_blue_volume(dmesh, vfunc, vc, crop, bounds, cam, domain_size; spp=256, bcx=0f0, bcy=0f0, bcz=0f0)
    i1, i2, j1, j2, k1, k2 = crop
    (bxr, byr, bzr) = bounds
    sx, sy, sz = domain_size
    xm = max(2f0, bxr[1]-15f0); xM = min(Float32(sx-1), bxr[2]+15f0)
    wym = byr[2] - 5f0; wyM = min(Float32(sy-2), byr[2]+80f0)
    zm = max(2f0, bzr[1]-15f0); zM = min(Float32(sz-1), bzr[2]+15f0)
    ...
    mesh!(s, dmesh; material=Hikari.CoatedDiffuse(
        reflectance=(0.30f0,0.35f0,0.45f0), roughness=0.02f0, eta=1.5f0))
    streamplot!(s, vfunc, xm..xM, wym..wyM, zm..zM;
        gridsize=(6,12,6), stepsize=0.4f0, maxsteps=800, density=0.6,
        use_tubes=true, tube_radius=0.09f0, tube_n_sides=6,
        tube_spline=true, tube_spline_resolution=3,
        color=dx -> log10(max(1f-4, norm(dx))) + 4f0,
        colormap=:inferno, arrow_size=0)
    volume!(s, Float32(i1)..Float32(i2), Float32(j1)..Float32(j2), Float32(k1)..Float32(k2), vc;
        colormap=[RGBA(0,0,0,0), RGBA(0,0,0,0), RGBA(0.01,0.02,0.08,0.005), RGBA(0.03,0.08,0.25,0.015),
                  RGBA(0.08,0.2,0.55,0.04), RGBA(0.15,0.38,0.8,0.08), RGBA(0.25,0.55,0.92,0.16),
                  RGBA(0.4,0.72,0.98,0.3), RGBA(0.6,0.88,1.0,0.48)],
        colorrange=(0.015f0,0.13f0),
        material=(extinction_scale=14f0, asymmetry_g=0.6f0, single_scatter_albedo=0.3f0))
    update_cam!(s, cam..., Vec3f(0,0,1))
    return colorbuffer(s; device=render_device,
        integrator=Hikari.VolPath(; samples=spp, max_depth=10, hw_accel=render_hw_accel),
        tonemap=:aces, gamma=2.2f0, exposure=2.2f0)
end

function panel_warm_hero(dmesh, vfunc, vc, crop, bounds, domain_size, bcx, bcy, bcz; spp=256)
    i1, i2, j1, j2, k1, k2 = crop
    (bxr, byr, bzr) = bounds
    sx, sy, sz = domain_size
    xm = max(2f0, bxr[1]-15f0); xM = min(Float32(sx-1), bxr[2]+15f0)
    ...
    s = Scene(size=(960,540);
        ...
        backgroundcolor=RGBf(0.012,0.02,0.05))
    cam3d!(s)
    mesh!(s, dmesh; material=Hikari.CoatedDiffuse(
    ...
    streamplot!(s, vfunc, xm..xM, wym..wyM, zm..zM;
        ...
        colormap=:inferno, arrow_size=0)
    volume!(s, Float32(i1)..Float32(i2), Float32(j1)..Float32(j2), Float32(k1)..Float32(k2), vc;
        colormap=[RGBA(0,0,0,0), RGBA(0,0,0,0), RGBA(0.03,0,0.08,0.006), RGBA(0.15,0.01,0.3,0.018),
                  RGBA(0.45,0.04,0.15,0.04), RGBA(0.75,0.12,0.03,0.09), RGBA(0.95,0.28,0.04,0.18),
                  RGBA(1.0,0.55,0.12,0.32), RGBA(1.0,0.78,0.32,0.5)],
        colorrange=(0.015f0,0.13f0),
        material=(extinction_scale=15f0, asymmetry_g=0.4f0, single_scatter_albedo=0.25f0))
    update_cam!(s, Vec3f(bcx+80,bcy-65,bcz+18), Vec3f(bcx-5,bcy+25,bcz), Vec3f(0,0,1))
    return colorbuffer(s; device=render_device,
        integrator=Hikari.VolPath(; samples=spp, max_depth=12, hw_accel=render_hw_accel),
        tonemap=:aces, gamma=2.2f0, exposure=2.8f0)
end
...
"""
function compute_pathlines_dye(data, bounds, bcx, bcy, bcz; n_per=0, n_steps=800, dt=0.5f0)
    u = data.u
    sx, sy, sz = size(u)[1:3]
    (bxr, byr, bzr) = bounds
    ...
    # Seed positions: grid covering body + wake region, similar density to streamplot gridsize
    seed_gx, seed_gy, seed_gz = 10, 18, 10
...
function panel_pathlines_dye(dmesh, dye_r, dye_g, dye_b, dye_lo, dye_hi, bcx, bcy, bcz; spp=256)
    s = Scene(size=(960,540);
        lights=[
            Makie.PointLight(RGBf(15000,12000,8000), Vec3f(bcx-50,bcy-90,bcz+60)),
        ...
        backgroundcolor=RGBf(0.005,0.01,0.03))
    cam3d!(s)
    mesh!(s, dmesh; material=Hikari.CoatedDiffuse(
        reflectance=(0.30f0,0.35f0,0.45f0), roughness=0.02f0, eta=1.5f0))

    for (ch, col) in [(dye_r,(1.0,0.0,0.0)), (dye_g,(0.0,1.0,0.0)), (dye_b,(0.0,0.0,1.0))]
        volume!(s, dye_lo[1]..dye_hi[1], dye_lo[2]..dye_hi[2], dye_lo[3]..dye_hi[3], ch;
            colormap=[RGBA(0,0,0,0), RGBA(col[1]*0.5,col[2]*0.5,col[3]*0.5,0.25), RGBA(col...,0.7)],
            colorrange=(0.05f0, 0.85f0),
            material=(extinction_scale=6f0, asymmetry_g=0.5f0, single_scatter_albedo=0.55f0))
    end

    # Side/front 3/4 view showing the dye streams entering from upstream
    update_cam!(s, Vec3f(bcx+60, bcy-80, bcz+25), Vec3f(bcx, bcy+20, bcz), Vec3f(0,0,1))
    return colorbuffer(s; device=render_device,
        integrator=Hikari.VolPath(; samples=spp, max_depth=10, hw_accel=render_hw_accel),
        tonemap=:aces, gamma=2.2f0, exposure=1.8f0)
end

function combine_4panel(img1, img2, img3, img4; border=4)
    h, w = size(img1)
    combined = fill(RGBf(0.08, 0.08, 0.1), 2h + border, 2w + border)
    combined[1:h, 1:w] .= img1
    combined[1:h, w+border+1:2w+border] .= img2
    combined[h+border+1:2h+border, 1:w] .= img3
    combined[h+border+1:2h+border, w+border+1:2w+border] .= img4
    return combined
end
...
if !isfile(cache_file)
    ...
    println("Running dolphin simulation on $(sim_mem) (scale=3, Re=1e5, tU/L=5)...")
    sim = dolphin(; scale=3f0, Re=1e5, mem=sim_mem)
    sim_step!(sim, 5; verbose=true, remeasure=false)
    save_sim(sim)
end

function render_all(; spp=512)
    data = load_sim()
    ...
    println("Rendering 4 panels at $(spp) spp...")
    println("  Panel 1: Dense streamlines...")
    img1 = panel_streamlines(dmesh, vfunc, bounds, cam_blue, domain_size; spp, bcx, bcy, bcz)
    println("  Panel 2: Blue volume + wake streamlines...")
    img2 = panel_blue_volume(dmesh, vfunc, vc, crop, bounds, cam_blue, domain_size; spp, bcx, bcy, bcz)
    println("  Panel 3: Warm hero + streamlines...")
    img3 = panel_warm_hero(dmesh, vfunc, vc, crop, bounds, domain_size, bcx, bcy, bcz; spp)
    println("  Panel 4: Colored pathlines dye injection...")
    dye_r, dye_g, dye_b, dye_lo, dye_hi = compute_pathlines_dye(data, bounds, bcx, bcy, bcz)
    img4 = panel_pathlines_dye(dmesh, dye_r, dye_g, dye_b, dye_lo, dye_hi, bcx, bcy, bcz; spp)

    combined = combine_4panel(img1, img2, img3, img4)
    save(output_file, combined)
    println("Saved: $output_file ($(size(combined)))")
    return combined
end

img = render_all(; spp=256)
